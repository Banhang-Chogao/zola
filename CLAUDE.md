...
- **Bài viết qua phím tắt `bb10`** (cùng nhánh `baochi`, `content/baochi/` — nguồn
  Wikipedia EN/VI) PHẢI có category mặc định `"Kiến thức"` (slug `kien-thuc`) thay
  cho `"Báo chí"` — đứng ngay sau `"Tất cả"`, trước category theo content. Ví dụ:
  `["Tất cả", "Khoa học", "Kiến thức"]`. Bài `bb10` LUÔN dẫn link bài Wikipedia gốc
  cuối bài (qua `[[extra.references_external]]`, CC BY-SA). Chi tiết: `shortcuts.md` §`bb10`.
