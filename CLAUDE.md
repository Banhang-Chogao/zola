...
#### V15 — Runtime Artifact Conflict: volatile state/log/report files dirty PRs

> Process + .gitignore vaccine. Match the signature → run the FIXER below; do NOT
> manually merge stale runtime JSON/log content.

- **Symptom:** A PR (especially `vaccine-hotfix/*` or `qa/*` branches) shows
  `mergeable_state: dirty` with conflicts clustered ONLY in:
  `data/*-state.json` · `data/*.log` · `data/vaccine-hotfix-report.json` ·
  `reports/rule-conflict-report.json` · `reports/rule-conflict-report.md`.
  The real feature/fix content has **no conflicts**. This is the exact conflict
  class that blocked PR #547 (20/06/2026).
- **Root cause:** These are **volatile runtime artifacts** — they are regenerated
  on EVERY QA/vaccine run. When a `vaccine-hotfix/*` branch is open and any
  vaccine or QA checker runs on `main`, both sides independently write a new
  version of the same file → merge conflict on every PR that touches them.
  The files were historically tracked in git (via `git add -A` in workflows),
  which is the source of the problem. They contain no canonical data needed by
  the Zola build (they are NOT referenced in any template).
- **Files that MUST NOT be committed:**
  - `data/*-state.json` — runtime state/lock for vaccine-autofixer, hotfix,
    autofix-conflicts, auto-merge-loop, qa-rule-checker
  - `data/*.log` — runtime logs (vaccine-autofixer.log, vaccine-hotfix.log)
  - `data/vaccine-hotfix-report.json` — per-run hotfix report (not in templates)
  - `reports/rule-conflict-report.json` / `.md` — generated by qa-auto-rule-checker
- **FIXER (conflict resolution):**
  1. `git checkout --theirs <file>` for each conflicting runtime artifact above.
  2. `git add <file>` to mark resolved.
  3. Continue with merge commit as normal.
  **Never hand-merge stale JSON/log/report content — always take `theirs` (main).**
- **FIXER (permanent prevention — already applied on 20/06/2026):**
  `.gitignore` now includes patterns `data/*-state.json`, `data/*.log`,
  `data/vaccine-hotfix-report.json`, `reports/rule-conflict-report.*`.
  Run `git rm --cached <file>` for any currently-tracked volatile artifact.
  Verify: `git ls-files data/*-state.json data/*.log` → must be empty.
- **Workflow fix:** `.github/workflows/vaccine-hotfix.yml` uses `git add -A`
  which previously picked up untracked volatile files. After gitignore update,
  `git add -A` no longer stages them → no more accidental commits of runtime state.
- **Detection (V15 detector in `scripts/qa_vaccines.py`):** checks that:
  (a) `.gitignore` contains the required volatile-file patterns, and
  (b) none of the volatile artifact files are currently tracked by git.
  FAIL if a known volatile file is still tracked (reverts the fix). WARN if
  `.gitignore` is missing the patterns (risk of re-introduction).
- **Tests:** `python3 -m unittest scripts.test_qa_vaccines -v` — class
  `RuntimeArtifactVaccineTest` covers: tracked file triggers FAIL, ignored
  untracked file → PASS, missing .gitignore pattern → WARN.
- **Validation (20/06/2026):** After fix, `git ls-files data/*-state.json data/*.log`
  = empty; zola build PASS; qa_check PASS; no conflict markers anywhere.
