...
##### V5b — rate limit ở `actions/deploy-pages` (deploy job), KHÔNG chỉ configure-pages (27/06/2026)

- **Dấu hiệu:** **build job PASS**, `configure-pages` PASS, nhưng **deploy job** đỏ ở
  step `actions/deploy-pages@v5`: `Creating Pages deployment failed` →
  `HttpError: API rate limit exceeded for installation` → `Failed to create deployment
  (status: 403)`. Khác V5 gốc (lỗi ở `configure-pages`/build job).
- **Nguyên nhân:** fix V5 gốc chỉ bọc retry quanh **configure-pages** (build job). Nhưng
  `actions/deploy-pages` gọi `createPagesDeployment` (POST `/pages/deployments`) **KHÔNG
  có retry nội bộ** cho call tạo deployment → khi quota API theo giờ của installation cạn
  (bão deploy: bot compliance/merge-report/security-audit… push `main` liên tục, mỗi push
  trigger 1 deploy), deploy job đỏ ngay dù build OK.
- **FIXER (đã áp — branch `claude/github-pages-rate-limit-x8pcdh`):** trong `deploy.yml`
  job `deploy`, thay 1 step `actions/deploy-pages@v5` đơn lẻ bằng **chuỗi 3 attempt
  exponential backoff (0 → 60s → 120s)**, CHỈ dùng action chính thức (KHÔNG thêm
  dependency). 2 attempt đầu `continue-on-error: true` (nuốt lỗi để retry); attempt cuối
  KHÔNG nuốt lỗi → lỗi thật vẫn nổi. `environment.url` coalesce
  `steps.deploy_attempt_1/2/3.outputs.page_url` (toán tử `||` trả truthy đầu tiên). Mỗi
  attempt xin OIDC token mới nên lặp lại an toàn; deployment chỉ tạo khi quota đã hồi.
  Đồng thời `configure-pages` xử lý 409 "already exists" như **idempotent success** (bớt
  log lỗi giả mỗi deploy).
- **Quy tắc chung:** rate limit installation ảnh hưởng **mọi** call Pages API trong cùng
  run (cả configure lẫn deploy) → đặt retry/backoff ở **cả hai** job, không chỉ một. Khi
  thấy "API rate limit exceeded for installation" → xác định **step nào** đỏ (build vs
  deploy) trước khi sửa.
- **Validation:** `python3 scripts/validate_workflows.py` (34/34 valid) + `python3
  qa_check.py` PASS; deploy job tự retry khi quota cạn thay vì đỏ ngay.
...
#### V17 — Changelog Backend Migration (REVERTED 28/06/2026 — PR #1201)

> Changelog đã revert về file-based (`changelog.json`), public, no admin gate.
> Vaccine này được giữ làm historical reference nhưng KHÔNG còn hiệu lực.

- **Lịch sử:** `changelog.json` từng được migrate lên VIPZone backend để tránh merge conflict (V17 cũ).
  Ngày 28/06/2026 revert lại file-based vì:
  - Conflict có thể quản lý bằng `--theirs` strategy (như V6/V18)
  - Backend gây phụ thuộc Render + `VIPZONE_ADMIN_TOKEN`
  - Changelog là dữ liệu public, không cần admin gate
- **Trạng thái hiện tại:** `changelog-update.yml` gọi `update_changelog.py` → push `changelog.json` vào
  main. Hoàn toàn public, không kiểm tra quyền admin. Code VIPZone changelog đã xoá (`changelog.py`,
  `migrate_changelog.py`, `push_changelog_entry.py`, changelog CRUD trong db.py, router mount trong main.py).
- **Rule:** Changelog là dữ liệu commit vào repo, public, file-based. KHÔNG dùng backend cho changelog.
  Nếu conflict → dùng strategy V6 (`git checkout --theirs changelog.json`).
...
- **`baochi` KHÔNG phải category — nó là NGUỒN/LOẠI bài** (cập nhật 27/06/2026,
  ghi đè rule cũ "Báo chí cho `bb`"). Bài viết qua phím tắt `bb` (nhánh `baochi`,
  `content/baochi/`):
  - `[taxonomies] categories` PHẢI là **chuyên mục NỘI DUNG thật** suy ra từ nội
    dung (title/body/keywords): `"Tất cả"` đầu mảng + chuyên mục thật. Ví dụ bài
    ngân hàng → `["Tất cả", "Ngân hàng"]`, KHÔNG phải `["Tất cả", "Báo chí", …]`.
  - Dấu nguồn ghi ở `[extra]`: `source = "bb"`, `content_origin = "baochi"`
    (+ tuỳ chọn `original_source_type`, `category_confidence`, `category_reason`).
    `baochi` chỉ để truy vết/lọc nội bộ, **KHÔNG** là chuyên mục khám phá.
  - Confidence < 0.65 → fallback an toàn `"Đời sống"`. **KHÔNG bao giờ** default
    `"Báo chí"`. **KHÔNG** tự gán `premium`.
  - Bộ phân loại + backfill chuẩn: `scripts/audit_category_mapping.py`
    (dry-run mặc định; `--apply` để sửa file cũ). Test:
    `python3 -m unittest scripts.test_audit_category_mapping -v`.
  - Homepage "Khám phá"/filter đọc **taxonomy/category thật**, KHÔNG đọc
    folder/path/source. `"Báo chí"` đã gỡ khỏi filter chip + navbar + posts.
- **Bài viết qua phím tắt `bb10`** (cùng nhánh `baochi`, nguồn Wikipedia EN/VI) có
  category nội dung mô tả tri thức `"Kiến thức"` (slug `kien-thuc`) — đứng ngay sau
  `"Tất cả"`, kèm chuyên mục theo content. Ví dụ: `["Tất cả", "Khoa học",
  "Kiến thức"]`. Bài `bb10` LUÔN dẫn link Wikipedia gốc cuối bài
  (`[[extra.references_external]]`, CC BY-SA). Chi tiết: `shortcuts.md` §`bb10`.
- Danh sách category hợp lệ cho editor/CMS khai báo trong `categories.json`
  (`"Báo chí"` đã gỡ — chỉ thêm lại nếu có chuyên mục biên tập thật về báo chí).
