...
## SEOMONEY Editorial UI Spec

### Typography stack rules
- Primary: `Ericsson Hilda` (self-hosted, brand typeface) for headings and body.
- Fallback chain: `Manrope` → `Be Vietnam Pro` → `Inter` → system-ui.
- Vietnamese-glyph safety: Ericsson Hilda may not cover all Vietnamese diacritics. Be Vietnam Pro is the mandated Vietnamese-safe fallback. Any heading/body text containing Vietnamese must render legibly — test after font changes.
- Hilda/display font usage: Hilda is allowed for English/no-diacritic labels, editorial display words, and brand accents. Do NOT use Hilda for Vietnamese paragraph/headline text if diacritics appear broken. Use Be Vietnam Pro or system-ui for Vietnamese-safe rendering.
- Code/terminal: keep `ui-monospace`, `SF Mono`, `Fira Code`, `JetBrains Mono` stack.
- Hard rule: Do not bundle or reference proprietary fonts unless they already exist in the repo and are intended for site use. Do not use random downloaded fonts or unlicensed commercial fonts.

### Icon rules
- Follow S-DNA/B-DNA: use minimal inline SVG or existing internal icon conventions (e.g. search magnifying glass, close X).
- Do not import random external icon packs.
- Search icon: a single refined magnifying-glass SVG only (circle + diagonal line), stroke-width 2, no extra decorations.
- Keep icons minimal — no gradients, no emoji replacements for functional icons.

### Homepage editorial layout spec
- Max content width: 1180–1280px (handled by `.container` layout).
- Three-column feature grid (desktop):
  - Left rail (280–320px): compact article list. Thumbnails 120px wide, ratio 16:10. Clear horizontal `<hr>` dividers between items. Show category eyebrow + title + date.
  - Center (1fr): lead story. Large thumbnail (16:10 ratio), strong headline (800 weight, clamp size), short description, author + date meta.
  - Right rail (300–340px): stacked secondary stories. Full-width thumbnail (16:9 ratio), category eyebrow, title, date. Items separated by bottom border.
  - Gap: 24px between columns.
- Below feature grid: horizontal feed with enhanced card boundaries:
  - Cards use grid layout (240px thumbnail + description) with bottom border dividers.
  - No background fills, no box-shadow on feed cards — clean editorial list style.
- Mobile (≤960px): stack all columns vertically. Order: lead story → left compact list → right stacked.
- No horizontal overflow at any breakpoint.

### Card boundary rule
- Every post card/list item must have a clear visual boundary: bottom border (`1px solid var(--c-border)`) or `.home-card` divider.
- Do not use flat cards with no separation.
- Use subtle borders, not heavy box-shadows, for editorial tone.

### Search UI rule
- Modal overlay with backdrop blur.
- Minimal search field with inline magnifying-glass SVG icon inside the input.
- No standalone search button (input triggers on Enter).
- Clean close button (X icon) in top-right.
- No "Search nội bộ" or redundant eyebrow labels.
- Results shown as compact cards with title + URL + snippet.

### No random external assets rule
- Do not import random CSS frameworks, icon packs, or font files.
- All UI must use existing S-DNA/B-DNA tokens and components.
- If a visual need is not covered by existing tokens, extend tokens — do not add ad-hoc colors or spacings.

### Mobile-first scanability rule
- Each article/tappable unit must be visually distinct and appropriately sized for touch (min 44px tap targets).
- No text clipping, no horizontal scroll.
- Image ratios must remain stable across breakpoints.

### Do not reintroduce old patterns
- No old welcome carousel/banner (`header-rotator` with arrows + dots).
- No POSTINGS/Hangul decorative wordmark banner.
- No Premium Post (`premium-posts`) sidebar block — all content is currently free.
- No `home-card__insight` ("Insights khán giả") label.
- No `site-search__eyebrow` ("Search nội bộ") label.
