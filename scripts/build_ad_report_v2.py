...
def _scan_posts(manifest: dict) -> tuple[list[dict], dict, bool]:
    ...
    for folder in CONTENT_DIRS:
        ...
        for md in sorted(folder.glob("*.md")):
            ...
            # FAQ schema = JSON-LD FAQPage generated by base.html from [[extra.faq]]
            # frontmatter (NOT inline qa_pair shortcodes, which only emit microdata).
            has_faq = "[[extra.faq]]" in text or "[extra.faq]" in text
            faq_count = text.count("[[extra.faq]]")
            faq_bonus = 6 if has_faq else 0
...
def _immediate_actions(pagespeed: dict | None, compliance: dict | None, ad_density: dict, ui_review: dict, posts: list[dict]) -> list[dict]:
    ...
    # SEO schema — data-driven: only recommend FAQ where the top posts GENUINELY
    # lack the JSON-LD FAQPage schema. Previously this action fired unconditionally
    # (paired with the broken detection below), so it nagged even when all top
    # posts already had FAQ. Now it reports real coverage.
    top = sorted(posts, key=lambda p: -p["monetization_score"])[:20]
    missing_faq = [p for p in top if not p.get("has_faq")]
    if missing_faq:
        names = ", ".join(p["title"][:40] for p in missing_faq[:3])
        actions.append({
            "priority": "P2",
            "action": "Add FAQ schema to top posts",
            "detail": (
                f"{len(missing_faq)}/{len(top)} top posts thiếu FAQ schema "
                f"([[extra.faq]] frontmatter → JSON-LD FAQPage). Thêm 3–5 Q&A bám "
                f"nội dung để tăng featured snippet + CTR. Ưu tiên: {names}."
            ),
        })
    elif top:
        covered = sum(p.get("faq_count", 0) for p in top)
        actions.append({
            "priority": "P3",
            "action": "Refine FAQ schema on top posts",
            "detail": (
                f"Top {len(top)} posts đã có FAQ schema ({covered} Q&A tổng) — rà "
                f"chất lượng answer, tránh nội dung generic để giữ featured snippet."
            ),
        })
...
def _posts_to_optimize(posts: list[dict], categories: list[dict]) -> list[dict]:
    ...
    for p in posts:
        if p["monetization_score"] >= 70:  # High potential
            ...
            # Missing FAQ schema. NOTE: the post dict never carried the raw body,
            # so the old `"[[extra.faq]]" not in str(p)` check was always true and
            # flagged EVERY high-value post — even ones with 4+ FAQ items. Use the
            # real `has_faq` flag captured during the content scan instead.
            elif not p.get("has_faq"):
                candidates.append({
                    "title": p["title"],
                    "url": p["url"],
                    "category": real_cat,
...
def build_report(*, force_full: bool = False) -> dict:
    ...
    immediate_actions = _immediate_actions(pagespeed, compliance, ad_density, ui_review, posts)
    ...
    top50 = [
        {
            ...
            "has_faq": p.get("has_faq", False),
            "faq_count": p.get("faq_count", 0),
        }
        for p in ranked[:50]
    ]
    top20 = ranked[:20]
