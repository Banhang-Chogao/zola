 ...
 * found the transaction defaults to TP. Hồ Chí Minh. Renders a Sembcorp-style
 ...
 *
 * Engine is identical across f/l/o-dashboard — only the global export name
...
(function (global) {
  ...
  function render(transactions, container) {
    ...
    const txns = Array.isArray(transactions) ? transactions : [];
    if (!txns.length) {
      container.innerHTML = '<p class="dash-geo__empty">Upload sao kê để xem phân bố giao dịch theo địa điểm.</p>';
      return;
    }
  ...
  global.FDashboardGeo = { analyze, detectPlace, render };
