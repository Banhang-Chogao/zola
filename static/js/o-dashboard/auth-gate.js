/**
 * O-Dashboard — GitHub OAuth gate (cùng flow CMS /editor/).
 * Dùng chung session `zola-cms-session-id` → đăng nhập CMS = vào được O-Dashboard.
 */
(function (global) {
  "use strict";
  ...
  function showView(name) {
    document.querySelectorAll("[data-od-view]").forEach((el) => {
      el.hidden = el.dataset.odView !== name;
    });
  }

  function populateUserBar(user) {
    const bar = document.querySelector("[data-od-user-bar]");
    if (!bar) return;
    const avatar = bar.querySelector("[data-od-user-avatar]");
    const name = bar.querySelector("[data-od-user-name]");
    const email = bar.querySelector("[data-od-user-email]");
  ...
  function showLoginError(code) {
    const el = document.querySelector("[data-od-login-error]");
    if (!el) return;
  ...
  async function init() {
    const root = document.getElementById("od-app");
    if (!root) return null;
    ...
    const loginBtn = root.querySelector("[data-od-action='github-login']");
    const logoutBtn = root.querySelector("[data-od-action='logout']");
    const hint = root.querySelector("[data-od-login-hint]");
