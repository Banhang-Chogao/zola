/**
 * H-Dashboard — load pdf.js with local-first + jsDelivr fallback (CSP-safe).
...
(function (global) {
  ...
  function loadScript(src) {
    return new Promise(function (resolve, reject) {
      ...
      const existing = document.querySelector('script[data-hd-pdfjs-src="' + src + '"]');
      if (existing) {
        if (global.pdfjsLib) {
          resolve();
          return;
  ...
  async function ensureReady() {
    ...
    const pdfCandidates = [metaContent("hd-pdfjs-src"), CDN_PDF].filter(Boolean);
    const workerCandidates = [metaContent("hd-pdfjs-worker"), CDN_WORKER].filter(Boolean);
    ...
    readyPromise = (async function () {
      ...
      for (let i = 0; i < pdfCandidates.length; i++) {
        try {
          ...
          loadErr = err;
          console.warn("[H-Dashboard] PDF.js source failed:", pdfCandidates[i], err);
        }
      }

      if (!global.pdfjsLib) {
        const msg =
          "pdf.js chưa tải — không thể đọc PDF hóa đơn. Kiểm tra mạng hoặc tải lại trang.";
