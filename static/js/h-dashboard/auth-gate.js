/**
 * H-Dashboard — GitHub OAuth gate (cùng flow CMS /editor/).
 * Dùng chung session `zola-cms-session-id` → đăng nhập CMS = vào được H-Dashboard.
 */
(function (global) {
  "use strict";
  ...
  function showView(name) {
    document.querySelectorAll("[data-hd-view]").forEach((el) => {
      el.hidden = el.dataset.hdView !== name;
    });
  }

  function populateUserBar(user) {
    const bar = document.querySelector("[data-hd-user-bar]");
    if (!bar) return;
    const avatar = bar.querySelector("[data-hd-user-avatar]");
    const name = bar.querySelector("[data-hd-user-name]");
    const email = bar.querySelector("[data-hd-user-email]");
  ...
  function showLoginError(code) {
    const el = document.querySelector("[data-hd-login-error]");
    if (!el) return;
  ...
  async function init() {
    const root = document.getElementById("hd-app");
    if (!root) return null;
    ...
    const loginBtn = root.querySelector("[data-hd-action='github-login']");
    const logoutBtn = root.querySelector("[data-hd-action='logout']");
    const hint = root.querySelector("[data-hd-login-hint]");
