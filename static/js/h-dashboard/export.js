/**
 * H-Dashboard export — auto-download JSON/PDF, then wipe local storage.
...
(function (global) {
  ...
  const BLOG_URL = (function () {
    ...
    return (location.origin + location.pathname).replace(/\/tools\/h-dashboard\/?$/, "");
  })();
  ...
  function drawDigitalSignature(doc, payload, pageW, margin) {
    ...
    setRgb(doc, INK);
    const certId = "HD-" + String(payload.series_id || "").slice(0, 8).toUpperCase();
  ...
  function drawHealthTiers(doc, x, colW, y) {
    ...
    const levels = global.HDashboardInsights.HEALTH_LEVELS || [];
    levels.forEach((lv) => {
    ...
    doc.autoTable({
      ...
      didDrawPage: (data) => {
        if (data.pageNumber > 1) {
          ...
          setRgb(doc, MUTED);
          doc.text("H-Dashboard — Bảng giao dịch (tiếp)", margin, 8);
        }
      },
      didParseCell: (data) => {
  ...
  async function exportCsv(transactions, insightsPayload) {
    ...
    await downloadBlob(blob, `h-dashboard-${stamp}-${payload.series_id}.csv`);
    return payload;
  }
  ...
  async function exportJson(transactions, insightsPayload) {
    ...
    await downloadBlob(blob, `h-dashboard-${stamp}-${payload.series_id}.json`);
    return payload;
  }
  ...
  async function exportPdf(transactions, insightsPayload) {
    ...
    const stamp = payload.exported_at.slice(0, 10);
    doc.save(`h-dashboard-report-${stamp}-${payload.series_id}.pdf`);
  ...
  async function mergePdfReports(fileList) {
    ...
    merged.setTitle("H-Dashboard — Báo cáo tích lũy");
    merged.setSubject("Gộp " + sourceNames.length + " báo cáo PDF");
    merged.setCreator("H-Dashboard · " + BLOG_URL_SLUG);
    ...
    await downloadBlob(blob, "h-dashboard-merged-" + stamp + ".pdf");
