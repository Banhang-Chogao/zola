...
Generated: `2026-06-26T21:02:16.167464+00:00`
...
- Before: green 105, orange 44, red 17
- After: green 105, orange 46, red 17

## Fixed posts

- `lich-trinh-busan-tu-tuc-3-ngay-2-dem`: 0.68 → **0.69** (mid → mid)
- `mcc-decoded-international-spending`: 0.42 → **0.42** (mid → mid)
- `mcc-travelers-cheat-sheet`: 0.45 → **0.46** (mid → mid)
- `mcc-masterclass-personal-strategy`: 0.38 → **0.39** (low → low)
- `bi-kip-xin-visa-han-quoc-5-nam-de`: 0.00 → **0.62** (low → mid)
- `bidv-smartbanking-khong-vao-duoc`: 0.00 → **0.54** (low → mid)
- `cac-truong-hop-khong-duoc-hoan-thue-gtgt`: 0.43 → **0.43** (mid → mid)
- `huong-dan-xac-thuc-cccd-msb-digital-bank`: 0.64 → **0.65** (mid → mid)
- `msb-digital-bank-cong-nghe-bao-mat`: 0.61 → **0.62** (mid → mid)
- `msb-digital-bank-ra-mat-thay-mbank`: 0.65 → **0.65** (mid → mid)
- `vpbank-circle-sieu-the-free-tron-doi-gen-z`: 0.52 → **0.52** (mid → mid)
...
- `phu-am-cang-bat-hoi-tieng-han` [mid] score=0.68 — Trung bình top-5 = 0.68 < 0.70 — cần thêm bài cùng intent hoặc làm sâu nội dung cluster.
...
- `kha-nang-kinh-nghiem-tieng-han` [mid] score=0.66 — Trung bình top-5 = 0.66 < 0.70 — cần thêm bài cùng intent hoặc làm sâu nội dung cluster.
...
- `an-khuya-sai-gon` [mid] score=0.62 — Trung bình top-5 = 0.62 < 0.70 — cần thêm bài cùng intent hoặc làm sâu nội dung cluster.
- `bi-kip-xin-visa-han-quoc-5-nam-de` [mid] score=0.62 — Trung bình top-5 = 0.62 < 0.70 — cần thêm bài cùng intent hoặc làm sâu nội dung cluster.
...
- `qa-gatekeeper-tu-fix-loi-blog` [mid] score=0.58 — Trung bình top-5 = 0.58 < 0.70 — cần thêm bài cùng intent hoặc làm sâu nội dung cluster.
...
- `troi-mua-sai-gon-an-gi` [mid] score=0.57 — Trung bình top-5 = 0.57 < 0.70 — cần thêm bài cùng intent hoặc làm sâu nội dung cluster.
...
- `lam-theme-log-sua-menu-cong-cu` [mid] score=0.56 — Trung bình top-5 = 0.56 < 0.70 — cần thêm bài cùng intent hoặc làm sâu nội dung cluster.
...
- `premium-fintech-paywall-demo` [mid] score=0.48 — Trung bình top-5 = 0.48 < 0.70 — cần thêm bài cùng intent hoặc làm sâu nội dung cluster.
- `mcc-rewards-cashback-international` [mid] score=0.46 — Trung bình top-5 = 0.46 < 0.70 — cần thêm bài cùng intent hoặc làm sâu nội dung cluster.
- `mcc-travelers-cheat-sheet` [mid] score=0.46 — Trung bình top-5 = 0.46 < 0.70 — cần thêm bài cùng intent hoặc làm sâu nội dung cluster.
...
- `mcc-fraud-blocks-international` [mid] score=0.44 — Trung bình top-5 = 0.44 < 0.70 — cần thêm bài cùng intent hoặc làm sâu nội dung cluster.
...
- `mcc-dispute-process-template` [mid] score=0.43 — Trung bình top-5 = 0.43 < 0.70 — cần thêm bài cùng intent hoặc làm sâu nội dung cluster.
...
- `mcc-corporate-compliance-expenses` [mid] score=0.41 — Trung bình top-5 = 0.41 < 0.70 — cần thêm bài cùng intent hoặc làm sâu nội dung cluster.
...
- `mcc-merchant-choosing-code` [mid] score=0.41 — Trung bình top-5 = 0.41 < 0.70 — cần thêm bài cùng intent hoặc làm sâu nội dung cluster.
...
- `mcc-cross-border-fees-interchange` [low] score=0.40 — Trung bình top-5 = 0.40 < 0.70 — cần thêm bài cùng intent hoặc làm sâu nội dung cluster.
- `mcc-masterclass-personal-strategy` [low] score=0.39 — Trung bình top-5 = 0.39 < 0.70 — cần thêm bài cùng intent hoặc làm sâu nội dung cluster.
...
- `hieu-ve-gioi-han-su-dung-va-gioi-han-do-dai-tren-claude` [low] score=0.35 — Top semantic score thấp (0.39) — nội dung quá khác biệt trong cluster.
- `muc-dong-bhyt-ho-gia-dinh-2026` [low] score=0.35 — Top semantic score thấp (0.41) — nội dung quá khác biệt trong cluster.
- `sentence-transformers-sbert-deep-dive` [low] score=0.34 — Top semantic score thấp (0.34) — nội dung quá khác biệt trong cluster.
- `macos-27-golden-gate-co-gi-moi` [low] score=0.33 — Top semantic score thấp (0.42) — nội dung quá khác biệt trong cluster.
- `ten-mien-ai-hon-dao-anguilla-hot-bac` [low] score=0.33 — Top semantic score thấp (0.46) — nội dung quá khác biệt trong cluster.
- `vneid-2-2-8-phan-anh-lua-dao-quet-qr` [low] score=0.32 — Top semantic score thấp (0.37) — nội dung quá khác biệt trong cluster.
