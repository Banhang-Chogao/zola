...
Với một blog tĩnh như blog này — được [tạo bằng Zola](/tao-blog-voi-zola/) và [tự động deploy bằng GitHub Actions](/tu-dong-deploy-zola-github-actions/) — Git chính là xương sống: mỗi bài viết, mỗi sửa đổi giao diện đều là một commit, mỗi tính năng là một nhánh.
...
Luồng cơ bản luôn là: **sửa file → `git add` → `git commit`**. Hiểu mô hình ba khu vực này là chìa khóa để không lúng túng khi mới học. Chúng ta sẽ thực hành kỹ ở [Bài 3 về các lệnh Git cơ bản](/lenh-git-co-ban-init-add-commit-status/).
...
Nói cách khác: Git là động cơ, GitHub là một trong nhiều "garage" để gửi xe và cộng tác (bên cạnh GitLab, Bitbucket…). Chúng ta sẽ tìm hiểu GitHub kỹ từ [Bài 7](/github-la-gi-tao-repository-dau-tien/).
...
- **Bài 2**: [Cài đặt Git và cấu hình lần đầu](/cai-dat-git-cau-hinh-lan-dau/).
- **Bài 3**: [Các lệnh cơ bản: init, add, commit, status, log](/lenh-git-co-ban-init-add-commit-status/).
