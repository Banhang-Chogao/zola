...
> 📚 **Git & GitHub Series (Bài 2/15)** — Sau khi đã hiểu [Git là gì ở Bài 1](/git-la-gi-version-control-cho-nguoi-moi/), bài này hướng dẫn **cài đặt Git** và cấu hình lần đầu để sẵn sàng commit.
...
> ⚠️ Email này nên trùng với email tài khoản GitHub để commit được liên kết đúng với hồ sơ của bạn về sau (sẽ dùng ở [Bài 7](/github-la-gi-tao-repository-dau-tien/)).
...
Nếu thấy lời chào `Hi <username>!` nghĩa là thành công. Chúng ta sẽ dùng SSH thật sự khi [làm việc với remote ở Bài 6](/git-remote-lam-viec-voi-repository-tu-xa/).
...
Ở **Bài 3**, chúng ta sẽ tạo repository đầu tiên và thực hành [các lệnh Git cơ bản: init, add, commit, status, log](/lenh-git-co-ban-init-add-commit-status/) — bắt đầu ghi lại lịch sử dự án thật sự. Nếu chưa đọc nền tảng, hãy quay lại [Bài 1: Git là gì](/git-la-gi-version-control-cho-nguoi-moi/).
