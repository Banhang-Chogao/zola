...
> 📚 **Git & GitHub Series (Bài 12/15)** — Sau bộ phao cứu sinh [stash, cherry-pick, reflog ở Bài 11](/git-stash-cherry-pick-reflog-nang-cao/), bài này dạy cách quay ngược thời gian có chủ đích với **git reset** và revert.
...
Lệnh này lùi một commit **và xóa sạch** mọi thay đổi chưa commit. Nó cực mạnh nhưng nguy hiểm. Trước khi `--hard`, hãy chắc chắn bạn không cần các thay đổi hiện tại — hoặc đã `git stash` chúng. Nếu lỡ tay, nhớ tới [git reflog ở Bài 11](/git-stash-cherry-pick-reflog-nang-cao/) để cứu hộ.
...
Ở **Bài 13**, chúng ta nâng tầm lên quy trình tổ chức công việc: [Git workflow chuyên nghiệp với Git Flow và GitHub Flow](/git-workflow-chuyen-nghiep-gitflow-github-flow/).
