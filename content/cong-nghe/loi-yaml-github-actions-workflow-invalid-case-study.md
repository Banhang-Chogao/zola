...
aliases = ["/loi-yaml-github-actions-workflow-invalid-case-study/",
  "/posting/loi-yaml-github-actions-workflow-invalid-case-study/"
]
...
Trước đó blog đã gặp [git stale branch mất common ancestor với main](/posting/git-stale-branch-root-cause-analysis-2026/). Nhánh tạo từ commit cũ, `main` bị force-push, `git merge-base` fail — CI đỏ sau vài phút build vô ích. Giải pháp là workflow **Check branch ancestry**: chạy sớm trên mỗi pull request, gọi `git merge-base HEAD origin/main`, fail nhanh và comment hướng dẫn rebase.
...
- Gate ancestry lại nằm đúng vị trí trong chuỗi phòng ngừa stale branch (bổ sung cho tài liệu [ROOT-CAUSE stale branch](/posting/git-stale-branch-root-cause-analysis-2026/)).
