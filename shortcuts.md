...
**Liên quan**: Vaccine **V16** (CLAUDE.md §4) · `wip8` (TheoDoi8 feed) surface `BACKEND_OUTDATED` sau deploy.
...
**Mục đích**: 1 shortcut gộp **WIP** (trạng thái workspace: git status, file thay đổi,
commits gần nhất, open PRs) **+ TheoDoi8** (snapshot CI/CD các commit gần nhất) để **tái
dựng task đang dở** mà không cần user nhớ lại. **READ-ONLY hoàn toàn**: không sửa file,
không commit, không push, không mở PR, không deploy; TheoDoi8 chỉ **reuse data CI tự
sinh** (`data/theodoi8-report.json`) — không gọi network, không token/secret.

> **Gộp shortcut (2026-06-21):** `theodoi8` cũ đã **gộp vào `wip8`** — không còn shortcut
> riêng. Gọi `wip8` → xuất **2 bảng MD**: `## WIP8 table` + `## TheoDoi8 table`.

**Khác các shortcut gần giống**:
- `tieptuc8` — quét backlog rồi **tiếp tục/hoàn tất** task dở (có write operations).
- `runner` — retry lệnh/workflow đang fail.
- `wip8` — **read-only snapshot** (WIP + CI feed): reconstruct "mình đang làm gì?" + soi
  trạng thái commit/CI gần nhất rồi dừng lại, không can thiệp.

**Hành động** (thứ tự):

> **Thu thập dữ liệu (1 lần, KHÔNG gọi trùng):** chạy `python3 scripts/wip8.py --data`
> (in JSON git status/diff/log/stash/branch + `theodoi8` CI feed, READ-ONLY). Field
> `theodoi8` lấy từ report CI có sẵn `data/theodoi8-report.json` — **reuse, không fetch
> lại, không token/secret**. **Render ra user = MARKDOWN GitHub-flavored** (harness tự
> vẽ bảng header xám + link xanh + emoji) — KHÔNG dán ANSI box của `rich` (xấu trong UI
> chat). `rich`/`pyfiglet` chỉ dùng khi chạy local terminal thật (`python3 scripts/wip8.py`).
...
10. **TheoDoi8 CI feed** — đọc field `theodoi8` (status/summary/commits) từ cùng lần
    `wip8.py --data` ở trên (đã reuse `data/theodoi8-report.json`). KHÔNG poll lại GitHub.

**Output BẮT BUỘC = 2 bảng MD, đúng 2 heading cấp `##`:**

```
## WIP8 table
…(workspace: tổng quan + files changed + PR/CI — theo B-DNA bên dưới)…

## TheoDoi8 table
…(CI/CD: commit gần nhất × trạng thái run)…
```

- **Mobile-readable:** giữ số cột ít, message/file cắt gọn, không văn xuôi dài trong ô bảng.
- **Fallback khi thiếu data:** workspace sạch → `✅ Working tree sạch — 0 file thay đổi.`;
  không có report CI → `📡 Chưa có dữ liệu CI (data/theodoi8-report.json).` thay bảng TheoDoi8.
...
**`04 — 👀 THEODOI8`** · _purpose: snapshot CI/CD commit gần nhất (reuse data CI tự sinh)._

In dưới heading `## TheoDoi8 table`. Dòng đầu = 1-dòng tổng (`<icon> <status_label> ·
<summary> (HH:MM dd/mm/yyyy)`), rồi bảng:

| Commit | Message | Workflow | Trạng thái |
|---|---|---|---|
| `<sha7>` | <message cắt gọn> | <run_name> #<run_number> | 🔄 / ✅ / ❌ / ⊘ / ⏭ |

> Icon theo `theodoi8` report: 🔄 running · ✅ success · ❌ failure · ⊘ cancelled · 📡 idle.
> `cancelled` ≠ `failure` (concurrency — Vaccine V5). Không có commit/CI → in
> `📡 Chưa có dữ liệu CI (data/theodoi8-report.json).` thay bảng.
...
**Cú pháp mở rộng**:
- `wip8` — full workspace scan + TheoDoi8 CI feed (mặc định) → render 2 bảng markdown.
- `wip8 quick` — chỉ status + branch (bỏ log); vẫn kèm TheoDoi8 nếu có report.
...
- **Markdown-first**: output cho user là 2 bảng markdown GitHub-flavored (harness render
  bảng/link/emoji đẹp). ANSI `rich` chỉ cho terminal local, KHÔNG dán vào chat.
- **Icon đúng ngữ cảnh** — theo bảng trên; vaccine = 💉.
- **Không đoán mò / không fake data**: thiếu tín hiệu → báo `Không đủ context để suy task`;
...
- **Không gọi trùng**: TheoDoi8 reuse `data/theodoi8-report.json` — KHÔNG poll lại GitHub
  trong cùng lần `wip8`.
- **Chỉ chạy 1 lần** rồi dừng (snapshot WIP + CI tại thời điểm gọi) — gọi lại `wip8` để refresh.
