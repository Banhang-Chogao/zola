...
| `wip8` | Read-only snapshot — workspace (`## WIP8`) + CI/CD 1 lần (`## TheoDoi8`); gộp luôn `theodoi8` cũ, reuse `data/theodoi8-report.json` |
...
python3 scripts/backend_sha_check.py --json      # machine JSON (cho wip8 / Insights)
...
**Liên quan**: Vaccine **V16** (CLAUDE.md §4) · `wip8` (`## TheoDoi8`) surface `BACKEND_OUTDATED` sau deploy.
...
### `wip8` — Read-only snapshot: workspace + CI/CD (gộp `theodoi8` cũ)

**Mục đích**: Một shortcut **DUY NHẤT** tái dựng "mình đang làm gì?" từ workspace
(git status, file thay đổi, commits, stash, branch) **CỘNG** trạng thái CI/CD 1 lần
(commit gần nhất + workflow run + PR mở). **READ-ONLY hoàn toàn**: không sửa file,
không commit, không push, không mở PR, không deploy, KHÔNG re-trigger/rerun; phần
CI/CD chỉ **reuse report CI tự sinh** (`data/theodoi8-report.json`) — không gọi
network, không token/secret.

> ⚠️ **`theodoi8` đã bị gộp vào `wip8`** (2026-06-21 user request). KHÔNG còn shortcut
> `theodoi8` riêng, KHÔNG còn vòng lặp auto-refresh/poll. Phần CI/CD nay là **1
> snapshot** nằm trong section `## TheoDoi8` của output `wip8`. Cần live feed → gọi
> lại `wip8` (hoặc skill `/loop`, vd `/loop 1m wip8`).

**Khác các shortcut gần giống**:
- `tieptuc8` — quét backlog rồi **tiếp tục/hoàn tất** task dở (có write operations).
- `runner` — retry lệnh/workflow đang fail.
- `wip8` — **read-only snapshot** (workspace + CI feed): reconstruct "mình đang làm gì?"
  + soi trạng thái commit/CI gần nhất rồi dừng, không can thiệp.

**Thu thập dữ liệu**:

> **Thu thập dữ liệu (1 lần, KHÔNG gọi trùng):** `python3 scripts/wip8.py --data` (JSON
> git status/diff/log/stash/branch/health **+ field `theodoi8`**, READ-ONLY). Field
> `theodoi8` lấy từ report CI có sẵn `data/theodoi8-report.json` — **reuse, không fetch
> lại, không token/secret**. Cần tươi hơn report → MCP `list_commits` + `actions_list`
> (`list_workflow_runs`) + `list_pull_requests` làm **fallback** (run > token cap → lưu
> file rồi parse Python). **Render = MARKDOWN GitHub-flavored** (harness tự vẽ bảng/
> link/emoji) — KHÔNG dán ANSI `rich` (xấu trong chat); `rich`/`pyfiglet` chỉ cho
> terminal local.

Các bước thô (nếu không dùng `--data`):

1. `git status --short` · `git diff --stat HEAD` — file + quy mô thay đổi.
2. `git log --oneline -8` — commits gần nhất để suy feature đang làm.
3. `git stash list` + **branch context** (ahead/behind upstream).
4. **Open PR + running jobs** cho branch (MCP hoặc report CI).
5. **TheoDoi8 feed** — đọc field `theodoi8` (status/summary/commits) từ cùng lần
   `wip8.py --data` (reuse `data/theodoi8-report.json`). KHÔNG poll lại GitHub.

#### Output bắt buộc — ĐÚNG 2 section: `## WIP8` rồi `## TheoDoi8`

Theo design discipline [B-DNA](/tools/b-dna/) (calm enterprise: brand bar · health
meter · section có 1-dòng purpose · KPI nhấn số). **Mobile-safe (BẮT BUỘC):** mọi
bảng **tối đa 3 cột**, label ngắn, **truncate branch/title/message ~28 ký tự + `…`**,
ghi chú dài đưa **xuống dưới bảng** dạng blockquote — **KHÔNG bảng rộng**.

**Bộ ICON chuẩn:** 🔎 tổng quan · 🌿 branch · 📦 upstream · 📊 trạng thái · 🗂️ stash
· 📝 files · 🆕 new · ✏️ modified · 🗑️ deleted · 🔀 renamed · ❓ untracked · 🚧 blockers
· 🔧 next step · 🔗 PR · 🛰️ running job · 🚀 deploy · 💉 vaccine/hotfix · 🕒 GMT+7 ·
CI: 🔄 in_progress · ⏳ queued · ✅ success · ❌ failure · ⊘ cancelled · ⏭ skipped · ⚪ none.

##### Section 1 — `## WIP8` (workspace)

```
## WIP8
🕒 <HH:MM dd/mm/yyyy> (GMT+7) · read-only snapshot
**Health** ▰▰▰▰▰▰▰▰▰▰ 100% · ✅ CLEAN — <reason 1 dòng>
```

Health lấy từ `wip8.py --data` → `health` (`score/verdict/tone/meter/reason`).
Verdict: ✅ **CLEAN** (pass) · 🟡 **WIP/PENDING/BEHIND** (warn) · 🔴 **BLOCKED**
(fail, có conflict). Meter `▰`/`▱` (10 ô). `dirty` = đang làm (WIP), chỉ conflict = BLOCKED.

**`🔎 Tổng quan`** · _purpose: branch context + việc đang làm._ (2 cột)

| Mục | Giá trị |
|---|---|
| 🌿 Branch | `<branch…>` ↑N ↓M |
| 📦 Upstream | `<origin/…>` |
| 📝 Task | <1 câu suy từ diff + commit> |
| 🚧 Block | <chặn gì; "Không"> |
| 🔧 Next | <1 hành động tiếp> |

> 🗂️ Stash: <stash hoặc "không có">.

**`📝 Files`** · _purpose: thay đổi gom theo feature._ (2 cột; nhóm = note dưới bảng)

| Loại | File |
|---|---|
| ✏️ mod | `path/to/file` |

> Nhóm theo feature dưới bảng (vd `**shortcut merge:** shortcuts.md · wip8.py`).
> Sạch → in `✅ Working tree sạch — 0 file.` thay bảng.

##### Section 2 — `## TheoDoi8` (CI/CD snapshot 1 lần)

```
## TheoDoi8
🕒 <HH:MM dd/mm/yyyy> (GMT+7) · snapshot CI/CD (1 lần, KHÔNG poll)
```

**`🔗 PR & CI`** · _purpose: PR mở + pipeline cho branch._ (3 cột; branch = note dưới)

| PR | Title | CI |
|---|---|---|
| #X | <title…trunc> | 🟢/🟡/🔴/⚪ |

> Branch: `<branch…>`. Vaccine/hotfix → 💉 · deploy → 🚀.
> Không có → `Không có PR mở cho branch này.`

**`🛰️ Commits gần đây`** · _purpose: từng commit + run mới nhất._ (3 cột; msg truncate)

| Commit | Msg | TT |
|---|---|---|
| `a1b2c3d` | <msg…trunc> | 🔄/✅/❌/⊘ |

> Tóm tắt ≤1 dòng: `🔄 A chạy · ✅ X pass · ❌ Y fail · ⊘ Z huỷ (HH:MM dd/mm/yyyy GMT+7)`.
> `⊘ cancelled` ≠ fail nếu có run mới hơn `success` (concurrency — Vaccine V5).
> `❌ failure` thật trên `deploy.yml`/`qa` → báo + gợi ý `ff`, KHÔNG tự sửa trong `wip8`.
> Thiếu report CI → `📡 Chưa có dữ liệu CI (data/theodoi8-report.json).` thay bảng.

**Cú pháp mở rộng**:
- `wip8` — full snapshot (workspace + CI) → render 2 section markdown.
- `wip8 quick` — chỉ workspace status + branch (bỏ log + CI).
- `wip8 <path>` — chỉ inspect file/folder cụ thể (workspace).
- `wip8 <sha>` — soi đúng 1 commit + mọi run của nó trong `## TheoDoi8`.
- Local terminal có màu: `python3 scripts/wip8.py [--quick] [<path>]` (rich/pyfiglet).
- Lấy data JSON: `python3 scripts/wip8.py --data` (gồm cả field `theodoi8`).
- **Config TÙY CHỌN:** `data/shortcuts/wip8.config.json` (`show_theodoi8`,
  `theodoi8_max_commits`, `theodoi8_report_paths`). Thiếu file → dùng default. KHÔNG secret.

**Hard rules**:
- **READ-ONLY** — KHÔNG sửa/commit/push/deploy/mở PR/re-trigger. Chỉ đọc + in.
- **Đúng 2 section** `## WIP8` + `## TheoDoi8`, theo thứ tự đó.
- **Mobile-safe**: bảng ≤ 3 cột · label ngắn · truncate branch/title/msg ~28 ký tự +
  `…` · ghi chú dài xuống blockquote dưới bảng · KHÔNG bảng rộng.
- **Markdown-first**: output GitHub-flavored; ANSI `rich` chỉ cho terminal local.
- **Icon đúng ngữ cảnh** — theo bộ trên; vaccine = 💉.
- **Không đoán mò / không fake data**: thiếu tín hiệu → `Không đủ context để suy task`;
  thiếu report CI → fallback `📡 Chưa có dữ liệu CI` (KHÔNG bịa commit/CI).
- **Không gọi trùng**: TheoDoi8 reuse `data/theodoi8-report.json` — KHÔNG poll lại GitHub
  trong cùng lần `wip8`.
- **Chỉ chạy 1 lần** rồi dừng (snapshot tại thời điểm gọi; gọi lại `wip8` để refresh).
