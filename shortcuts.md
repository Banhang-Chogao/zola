...
| `wip8` | Read-only snapshot — workspace (`## WIP8`) + CI/CD 1 lần (`## TheoDoi8`); gộp luôn `theodoi8` cũ |
...
không commit, không push, không mở PR, không deploy, KHÔNG re-trigger/rerun.
...
- `wip8` — **read-only snapshot**: reconstruct workspace + CI 1 lần rồi dừng.

**Thu thập dữ liệu**:

> **Workspace:** `python3 scripts/wip8.py --data` (JSON git status/diff/log/stash/
> branch/health, READ-ONLY) HOẶC git thô (`status --short`, `diff --stat HEAD`,
> `log --oneline -8`, `stash list`, ahead/behind upstream). **CI/CD:** MCP
> `list_commits` (per_page=10 trên `main` + commit chưa merge của branch dev) →
> map `head_sha` qua `actions_list` (`list_workflow_runs`) + PR mở qua
> `list_pull_requests`. Kết quả run thường > token cap → lưu file rồi parse Python.
> **Render = MARKDOWN GitHub-flavored** (harness tự vẽ bảng/link/emoji) — KHÔNG dán
> ANSI `rich` (xấu trong chat); `rich`/`pyfiglet` chỉ cho terminal local.
...
- **Không đoán mò**: thiếu tín hiệu → `Không đủ context để suy task` thay vì bịa.
